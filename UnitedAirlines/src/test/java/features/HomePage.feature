Feature: SearchBox Functionality check

  @maria@pending
  Scenario: United Airlines searchBox functionality check with valid research
    Given I am on United Airlines homePage
    And I enter "Covid 19 Information" in searchBox
    When I click on search Button
    And I should see "COVID 19 Information" is properly displaying
    Then I verify "Search United.Com - Find Pages on Our Website & FAQs" in product title

  @maria@pending
  Scenario: United Airlines Travel Info is working properly
    Given I am on United Airlines homePage
    And I click on Travel Info TAb
    Then I should see "Travel Info" is properly displaying
#    And I verify "" in product title

  @maria@pending
  Scenario: United Airlines Book a ticket Info is working properly
    Given I am on United Airlines homePage
    And I enter {new york} in searchBox Info TAb
    And I enter {canada} in searchBox
    And I select a date
    And i click on find a flight
    Then I should see {trip} is properly displaying
    And i verify "" the product title

  @maria@pending
  Scenario: United Airlines contact us tab is working properly
    Given I am on United Airlines homePage
    And I click on contact us
    Then I verify the page title as Customer Support &amp; Service Portal


  @maria@pending
  Scenario: United Airlines contact us tab is working properly
    Given I am on United Airlines homePage
    And I click on contact us
    And I enter baggage fees in the searchbox
    Then I verify the page title as Search United.Com - Find Pages on Our Website & FAQs


